...
class MovieIssueReportList extends StatefulWidget {
  const MovieIssueReportList({super.key});

  @override
  State<MovieIssueReportList> createState() => _MovieIssueReportListState();
}

class _MovieIssueReportListState extends State<MovieIssueReportList> {
  static const int _pageSize = 5;

  late MovieIssueReportProvider _reportProvider;

  SearchResult<MovieIssueReport>? result;
  bool isLoading = true;
  int _page = 1;
  ...
  Future<void> _review(MovieIssueReport report) async {
  ...
  Widget _buildList() {
    ...
    final List<MovieIssueReport> reports = result?.items ?? List.empty();

    if (reports.isEmpty) {
      return Center(
        child: Text(
          "No movie issue reports found",
  ...
  Widget _buildCard(MovieIssueReport report) {
    return ReportCard(
      header: report.header ?? "-",
      subjectIcon: Icons.movie_filter_outlined,
      subject: _movieTitle(report),
  ...
  String _movieTitle(MovieIssueReport report) {
    final String title = report.movie?.title ?? "Unknown movie";
    final int? year = report.movie?.releaseDate?.year;
